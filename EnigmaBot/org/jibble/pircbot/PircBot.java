...
public abstract class PircBot implements ReplyConstants
	 ...
	 */
	public static final String VERSION = "1.5.0 modified";
	...
	public static final DateFormat timestamp = new SimpleDateFormat("HH:mm:ss.SSS");
	...
	public final void sendMessage(String target, String message)
		...
		if (split.length > 1)
			_outQueue.add("PRIVMSG " + target + " :" + split[0] + " \\n...");
		else
	...
	public void log(String line)
		{
		if (_outstream != null) _outstream.println(timestamp.format(new Date()) + " " + line);
		}
	...
	protected final void handleLine(String line)
		...
		if (command.equals("PONG"))
			{
			// The server has responded to our ping by sending a pong.
			this.onPong(line.substring(line.indexOf(" :") + 2));
			return;
			}

		//skip MOTD
		if (command.equals("372")) return;
		...
		if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001"))
			...
			this.onTopic(target,line.substring(line.indexOf(" :") + 2),sourceNick,
	...
	/**
	 * This method is called whenever we receive a pong from the server,
	 * presumably in response to a ping that we sent earlier.
	 *  <p>
	 * The implementation of this method in the PircBot abstract class
	 * performs no actions and may be overridden as required.
	 * 
	 * @param pingValue The value that was supplied as an argument to the PING command.
	 */
	protected void onPong(String pingValue)
		{
		}
	...
	/**
	 * Sets the verbose mode. If a stream is provided, then log entries
	 * will be printed to the stream. The default value is null and
	 * will result in no logging. For general development, we strongly recommend
	 * setting the verbose mode to some stream like System.out.
	 *
	 * @param out The stream to send log messages to.  Default is null for no logging.
	 */
	public final void setVerbose(PrintStream out)
		{
		_outstream = out;
		}
	 ...
	 */
	public static final int getMaxLineLength()
		{
	...
	public final User[] getUsers(String channel)
		...
		synchronized (_channels)
			...
			if (users != null) return users.toArray(new User[0]);
			}
		return new User[0];
		}
	...
	private final User removeUser(String channel, String nick)
		...
		synchronized (_channels)
			{
			Set<User> users = _channels.get(channel);
			if (users != null) return users.remove(user) ? user : null;
			}
		return null;
		}
			...
			if (users != null)
				...
				for (User userObj : users)
					...
					if (userObj.getNick().equalsIgnoreCase(nick))
						{
						if (userMode == OP_ADD)
							userObj.setPrefix(userObj.hasVoice() ? "@+" : "@");
						else if (userMode == OP_REMOVE)
							userObj.setPrefix(userObj.hasVoice() ? "+" : "");
						else if (userMode == VOICE_ADD)
							userObj.setPrefix(userObj.isOp() ? "@+" : "+");
						else if (userMode == VOICE_REMOVE) userObj.setPrefix(userObj.isOp() ? "@" : "");
						return;
						}
					}
				// Where'd this user come from?
				newUser = new User(userMode == OP_ADD ? "@" : (userMode == VOICE_ADD ? "+" : ""),nick);
				users.add(newUser);
				}
			else
				{
				System.out.println("No user info for this channel?");
				}
			}
		}
	...
	private Hashtable<String,Set<User>> _channels = new Hashtable<String,Set<User>>();
